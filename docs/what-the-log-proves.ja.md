...
| 2 | CDN は何を、どれだけの期間キャッシュせよと告げられたか？ | すべての donut 書き込みとリフレッシュ時の `cdn_headers` `{headers, surrogateKeys}` — 確定後に読み戻された、実際のレスポンスヘッダそのもの | 各フレーバーの暗黙のデフォルト（generic は `max-age=10`、Fastly/Akamai は `max-age=31536000`）と Akamai の `Surrogate-Key` → `Edge-Cache-Tag` へのリネームがピン留めされている。有効期間ヘッダが記録されていない = そのレスポンスは CDN に有効期間の指示を与えなかった（`putDonut` の種類） |
...
| 5 | なぜエントリがないのか — そのミスは cold か、それとも縮退か？ | `put_skipped` `{reason, code}`、`cache_error` `{operation, exceptionClass}` — `operation: read` が、それでも閉じる `cache_miss` と対になっているものが縮退した読み取り | スキップ理由、失敗した側（`read`/`write`）、throwable のクラスがピン留めされている。`cache_error{read}` + `cache_miss` = 縮退した読み取り、`cache_miss` 単独 = cold |
...
- **保持ポリシーが落としたセッション。** `DevQueryRepositoryLogModule` はすべてのセッションを
  ...
  本番では正常な読み取り — そして読み取り側プールの停止、これはプール自身の監視が先に
  捉えます — は意図的に存在しません。したがって「cold ではなく縮退」という読みは開発時の
  ものです。
