...
| 9 | Is this cache worth having? | `durationMs` on the `cache_hit` / `cache_miss` close - a hit measures serving from the pool, a miss measures the resource run and the write it triggered | The sign is the invariant, not the difference: `miss - hit` includes the fill, but a hit that is not faster than a miss is a cache costing money for nothing. Both closes carry it, a bare event carries null (no scope to measure), and the schema says it is a measurement rather than a contract |
