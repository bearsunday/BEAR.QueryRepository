...
- A resource that declares its own `Surrogate-Key` no longer loses the dependencies of the children it embeds. Introduced in 1.16.0: the declaration alone switched embed dependency resolution off, so the page kept serving a child that had been purged. Behaviour change for existing 1.16.x installations: such a page is now invalidated when an embedded child is, and its children's URI tags ride the `Surrogate-Key` header to the CDN. 1.15 and earlier resolved these dependencies too.
- A child tag was repeated in the `Surrogate-Key` header when a resource was written twice in one request (`#[Refresh]` puts the instance the rebuilding GET already stored). The stored tag sets were already deduplicated; the header a CDN receives now is too.
