...
- `SafeSemanticLogger(LogSinkInterface|null $sink = null, SessionStoreInterface $store = new ProcessSession())`: a process-wide facade whose open-scope depth and delegate logger live on the per-request `Session` the store resolves; no session crosses a serialized injector. Never breaking the host is the core logger's own guarantee since koriym/semantic-logger 0.9. `SafeSemanticLoggerProvider` binds it in whichever log module the app installs; without one, `QueryRepositoryModule` binds the core `Koriym\SemanticLogger\NullSemanticLogger`.
...
- `#[CacheLog]` (`BEAR\RepositoryModule\Annotation`): the qualifier the cache log is bound and injected with. `Koriym\SemanticLogger\SemanticLoggerInterface` belongs to koriym/semantic-logger and an application may already bind it (Be Framework records its becoming tree there), so this package no longer decides that binding. Resolve the cache log with `getInstance(SemanticLoggerInterface::class, CacheLog::class)`.
...
- `requestedTtl` field on `save_etag`, completing the save contexts; all `requestedTtl` descriptions state the convention that 31536000 is the `never` expiry placeholder and event-driven invalidation is the intended eviction path.
...
- `demo/run-degraded.php`: the failure vocabulary the other demos never reach, in thirteen sessions — `put_skipped` (all three reasons), a 4xx `command_result` with no invalidation, `save_view`, `manual_store`/`manual_invalidate`, `cache_error` (`read` and `write`), `invalidate` with `cdn` `purged`/`failed` and `roPool`/`etagPool` `failed`, `saved: false` on every save context, `manual_store_result{failed}`, all three `command` producers, both built-in command annotations (`#[Refresh]` and the purge-only `#[Purge]` shape), the Akamai flavor's renamed CDN headers and a finite `requestedTtl`. With it the demos emit every context type, every schema enum value and every command source the package can produce.
...
- `SessionStoreInterface`/`ProcessSession` (`BEAR\QueryRepository\Log`): the in-repo half of #179. `ProcessSession`, bound by both log modules, is one session per process - PHP-FPM/CLI behaviour is unchanged. A concurrent host binds a store keyed by its request context *and* a `LogSinkInterface` that flushes at its request end; the flush boundary stays the host's contract.
...
- The `update` parameter of `#[Cacheable]`: it has no effect and nothing reads it.
...
- `SaveDonutContext`/`SaveDonutViewContext`: the misleading `sMaxAge` field is renamed to `requestedTtl` — the value is the cache entry TTL, never a CDN s-maxage.
...
- One rule for what a cache failure does (#190): interceptors degrade what the cache path itself raised and record it; everything else the write touches keeps travelling. `Exception\CacheStoreFailure` marks the boundary, so a renderer with a bug is no longer swallowed by a `catch (Throwable)` that meant to forgive a pool outage. Two behaviour changes: an unreadable ETag pool now answers the conditional request in full instead of failing it, and a donut write that the store refuses serves the rendered page instead of a 500. A command or a direct call has no interceptor, so a CDN purge failure still reaches the caller.
- `#[Refresh]` no longer writes a `#[Cacheable]` destination a second time after the regenerating GET has stored it, which also removes one CDN purge per refresh.
- `ResourceDonut::FOMRAT` is renamed to `ResourceDonut::FORMAT`, and `EtagSetter::getEtagByEitireView()` to `getEtagByEntireView()`; the parameters `$httpCacche` (`EtagSetter::getEtagByPartialBody()`) and `$concheControlMaxAge` (`HeaderSetter::__invoke()`) are spelled `$httpCache` and `$cacheControlMaxAge`, which a named-argument caller has to follow.
...
- A donut write failure (pool outage or renderer error during `putStatic()`/`putDonut()` on the AOP path) propagated without any in-band record; it is now recorded as `cache_error{operation: write}`, matching the read side and the plain `#[Cacheable]` write side.
...
- A donut template is stored under its own URI tag, so `purge($uri)` evicts it (#185). A page that declares no `Surrogate-Key` used to store its template untagged, which no `invalidateTags()` call could reach: the purge dropped the content and the validator and left a shell that the next request recomposed and served. Behaviour change for existing installations - such a page now rebuilds on its first purge instead of being served from an immortal shell. Entries written before this version keep their empty tag list until they are rewritten (a refresh whose recomposed content changed re-tags them); anything never rewritten needs a cache clear to become reachable.
- A `#[Cacheable]` value entry is stored without rendering: a resource with no renderer in that context (an app resource under an html renderer) previously degraded every store to a warning and the cache stayed empty. ETag values change once for value entries (the validator falls back from the view to the body), and a stored value entry no longer carries the renderer's `Content-Type`. Custom `EtagSetterInterface` implementations now receive `$ro->view === null` on the value path and must read the body instead. A body containing something PHP cannot `serialize()` (a `Closure`, for instance) fails `EtagSetter::getEtagByEntireView()`'s body-derived validator outside `ResourceStorage::guard()`, so the write's `serialize()` `Exception` is not a `CacheStoreFailure` and propagates rather than degrading (#208's rule: only the store's own failure is swallowed). Keep such values out of a value entry's body, or catch `Exception` at the call site.
- `CliHttpCache::isNotModifiedFor()` read only `HTTP_IF_NONE_MATCH`, so the CLI request form (`argv[3]` = `If-None-Match="…"`) that `isNotModified()` accepts never answered 304 through the scoped path.
- `MatchQuery` threw `UnmatchedQuery` for an `onGet` parameter that has a default value but is absent from the command's query, so a write to a `#[Cacheable]` resource declared as `onGet(int $id, int $page = 1)` failed without `?page=`. Such a parameter is now omitted, and the regenerating `onGet` call receives the query by name so the omission cannot shift the remaining arguments.
- Two donut placeholders on one line of a template were captured as a single URI by a greedy pattern in `ResourceDonut`, and the refresh threw `ResourceNotFoundException` for the concatenated URI.
- `CDN-Cache-Control` directives were space-separated (`max-age=10 stale-while-revalidate=10`). RFC 9213 defines the field as a Structured Fields Dictionary, whose members are comma-separated, and requires a cache to ignore a field it cannot parse; the value is now `max-age=10, stale-while-revalidate=10`.
- Under `AkamaiModule`, a `#[CacheableResponse]` page lost its embedded children's invalidation tags: the Akamai setter renames `Surrogate-Key` to `Edge-Cache-Tag` before the storage read the header, so the page state and its ETag were stored under the page's own tag only and a child's purge never reached them. `DonutRepository` now hands the storage the keys as composed; `ResourceStorageInterface::saveDonutView()` gained an optional `$tags` parameter for that.
- `MobileEtagSetter` serialized the raw body, so a resource embedding another (`#[Embed]`) threw from `AbstractRequest::__serialize()` on every write; it now hashes the materialized copy the way `EtagSetter` does.
