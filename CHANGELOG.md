...
- A `#[Cacheable]` value entry is stored without rendering the resource: the render was work the write did not need, and a resource with no renderer in that context (an app resource under an html renderer) failed on every store - degraded to a warning, so responses stayed correct while the cache stayed empty. The validator falls back from the view to the body, so ETag values change once for value entries, and a stored value entry no longer carries the renderer's `Content-Type`. A custom `EtagSetterInterface` now receives `$ro->view === null` on the value path: an implementation that derives its tag from the view alone would collapse to one constant per URI and must read the body instead.
