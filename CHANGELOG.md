...
- A resource that declares its own `Surrogate-Key` still depends on the children it embeds: the declaration used to switch embed dependency resolution off, so the page kept serving a child that had been purged.
