...
- `onPost` on a `#[Cacheable]` class whose parameters did not cover `onGet`'s required ones threw `UnmatchedQuery`, uncaught (#219, a regression from #214 above): `RefreshSameCommand`'s automatic same-URI refresh runs `MatchQuery` against the write's own URI query, and a create posting to a collection URI routinely lacks a parameter `onGet` needs. The write itself already succeeded; the refresh had nothing to target. `RefreshSameCommand` now catches `UnmatchedQuery` for `onPost` specifically and records it as a `cache_error{operation: write}` instead, then lets `CommandsProvider`'s next command run - an explicit `#[Refresh]`/`#[Purge]` on the same method still fires. `onPut`/`onPatch`/`onDelete` keep throwing: those act on an entity `onGet` already addresses, so a missing required parameter there is a real signature mismatch (`BehaviorTest::testUnMatchQuery`).
