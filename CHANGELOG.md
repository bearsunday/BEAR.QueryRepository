...
- `SessionStoreInterface`/`ProcessSession` (`BEAR\QueryRepository\Log`): `SafeSemanticLogger` stays a process singleton, like `ServerContextInterface`/`ConcurrentRuntimeInterface`, but the open-scope depth and delegate logger it drives (now held on a small `Session` value object) are resolved per call through this store instead of living on the facade itself. `ProcessSession` is the bound default and answers with one session for the whole process, which is exactly right under PHP-FPM and the CLI, so that deployment's behaviour is unchanged. A concurrent host (a Swoole coroutine, a RoadRunner worker) binds its own `SessionStoreInterface` keyed by that host's request context, the same way it already binds `ServerContextInterface`; the store alone does not drain anything, so such a host must also bind a `LogSinkInterface` that flushes at its own request-end boundary rather than at process shutdown. This is the in-repo half of #179 — the host flush boundary for a concurrent runtime stays a host-side contract this package cannot discharge from inside a request.
...
### Changed
- `SafeSemanticLogger::__construct()` now takes `(LogSinkInterface|null $sink = null, SessionStoreInterface $store = new ProcessSession())`; it no longer accepts a `SemanticLoggerInterface` delegate. `new SafeSemanticLogger(new SemanticLogger())` becomes `new SafeSemanticLogger()`, and `new SafeSemanticLogger(new SemanticLogger(), $sink)` becomes `new SafeSemanticLogger($sink)`.
