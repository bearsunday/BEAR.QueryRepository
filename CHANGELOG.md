...
- The donut write skips any non-200 response, the threshold `#[Cacheable]` already applied (#190): it used to store anything below 400, so the same `203` was cached on one path and evicted on the other. Recorded as `put_skipped{reason: error-code}` with the actual code, as the `#[Cacheable]` side already does.
