...
- `ProdQueryRepositoryLogModule`: buffers each session and decides at flush, writing one JSON line per kept session to `php://stdout` or a file. `RetentionPolicyInterface`/`KeepMutationsAndFailures` keeps mutations (a `command` scope, a `manual_*` call, or an `invalidate` that is not marker-preceded cleanup), failures (`saved: false`, `cache_error`, `cdn`/`roPool`/`etagPool` `failed`, `put_skipped{error-code}`) and an optional sample; healthy reads are dropped, since a measured healthy session holds no failure entries at all. An app that wants full recording for one request implements the interface and overrides the binding.
- `LogSinkInterface`/`ShutdownFlush` and `LogWriterInterface`/`LogFileWriter`/`LogStreamWriter`: the log module owns flushing, so no app writes `flush()` in a bootstrap. The sink arms once per process — at construction and again at `SafeSemanticLogger::__unserialize()`, the only hook that runs on every request — and fires after output, after a 304's early `exit()` and after an uncaught error, which a hook on `ResourceObject::transfer()` cannot do (rendering happens inside it, and a 304 never reaches it). An empty session is never written.
- `ConcurrentRuntimeInterface`/`HostRuntime`: `ShutdownFlush` refuses to arm and warns on a runtime it can prove is concurrent (`RR_MODE` set, or inside a Swoole coroutine), where shutdown arrives once per worker and concurrent requests would share one session (#179). Mode is consulted, never capability — a loaded ext-swoole proves nothing about how the app is served.
