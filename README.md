...
- [Agent skill: reading the cache log](docs/skills/bear-cache-log/SKILL.md) — install it into a coding agent (`~/.claude/skills/`) so it observes the log instead of guessing: wiring, the event vocabulary, how a declaration is checked against what the log shows, app-vs-library triage, and what to report. Written in Japanese
